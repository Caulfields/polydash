    ...
    const MODELS = [
      { id: 'jma_seamless', label: 'JMA', color: '#f59e0b', note: 'Japan Meteorological Agency' },
      { id: 'ecmwf_ifs025', label: 'ECMWF IFS', color: '#34d399', note: 'ECMWF 0.25°' },
      { id: 'gfs_seamless', label: 'GFS', color: '#60a5fa', note: 'NOAA GFS' },
    ...
    function App() {
      ...
      const todayJmaRows = (rowsByModel.jma_seamless || []).filter(row => row.dateKey === nowParts.dateKey);
      const todayBestMatchRows = bestMatchRows.filter(row => row.dateKey === nowParts.dateKey);
      const selectedTodayRows = todayMode === 'average'
        ? todayAvgSeries
        : todayMode === 'main'
          ? (todayJmaRows.length ? todayJmaRows : todayBestMatchRows.length ? todayBestMatchRows : todayAvgSeries)
              ...
              <div style=${S.card}>
                ...
                <div style=${{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 10, flexWrap: 'wrap' }}>
                  ...
                  <div style=${{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    <button onClick=${() => setTodayMode('main')} style=${{ ...S.modeBtn, ...(todayMode === 'main' ? S.modeBtnActive : {}) }}>JMA</button>
                  </div>
                </div>
                <${TodayOverlayChart}
